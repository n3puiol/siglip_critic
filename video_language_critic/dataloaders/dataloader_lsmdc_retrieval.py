...
class LSMDC_DataLoader(Dataset):
    ...
    def __init__(
        ...
        video_json_path_dict["train"] = os.path.join(
            self.data_path, "LSMDC16_annos_training.csv"
        )
        video_json_path_dict["val"] = os.path.join(
            self.data_path, "LSMDC16_annos_val.csv"
        )
        video_json_path_dict["test"] = os.path.join(
            self.data_path, "LSMDC16_challenge_1000_publictect.csv"
        )
    ...
    def _get_captions_single(self, path):
        ...
        for pseudo_video_id, v_ in json_data.items():
            ...
            pseudo_caption_dict[pseudo_video_id]["start"] = np.array(
                [int(math.floor(float(itm[0]))) for itm in timestamps], dtype=object
            )
            pseudo_caption_dict[pseudo_video_id]["end"] = np.array(
                [int(math.ceil(float(itm[1]))) for itm in timestamps], dtype=object
            )
            pseudo_caption_dict[pseudo_video_id]["text"] = np.array(
                v_["sentences"], dtype=object
            )
        ...
        try:
            for i, video_id in enumerate(choice_video_ids):
                ...
                if len(raw_video_data.shape) > 3:
                    ...
                    print(
                        "video path: {} error. video id: {}".format(
                            video_path, video_id
                        )
                    )
